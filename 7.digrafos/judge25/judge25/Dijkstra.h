...
public:
	Dijkstra(DigrafoValorado<Valor> const& g, int orig) : origen(orig),
		dist(g.V(), INF), ult(g.V()), pq(g.V()), aristasDeDistancia(g.V(), 0){	//O( A log V)
...
private:
	const Valor INF = 2000000001;
	int origen;
	std::vector<Valor> dist;
	std::vector<Valor> aristasDeDistancia;
	...
	void relajar(AristaDirigida<Valor> a) {
		...
		if (dist[w] > dist[v] + a.valor()) {
			...
			aristasDeDistancia[w] = aristasDeDistancia[v]+1;
		}
		if (dist[w] == dist[v] + a.valor()) {
			if (aristasDeDistancia[w] >  aristasDeDistancia[v] + 1) {
				dist[w] = dist[v] + a.valor(); ult[w] = a;
				pq.update(w, dist[w]);	//O(log V)	push si no existia else flotar/hundir
				aristasDeDistancia[w] = aristasDeDistancia[v] + 1;
			}
