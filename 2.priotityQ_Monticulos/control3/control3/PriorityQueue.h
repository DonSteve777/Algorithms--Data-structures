...
public:
   ...
   /*
   Es completo y por tanto semicompleto si al descomponerlo en sus 2 hijos,
   estos son completos y tienen la misma altura
   */
   bool completoAux(int i, int & altura) const {
	   int alturaIzq, alturaDer;
	   int hijo = 2 * i; // hijo izquierdo, si existe
	   if (hijo <= size())
		   alturaIzq = altura + 1;
	   if (hijo+1 <= size())
		   alturaDer = altura + 1;
	   return alturaDer == alturaIzq &&
		   completo(hijo, alturaIzq) && completo(hijo + 1, alturaDer);
   }

   bool completo() const {
	   if (size() == 0) return true;
	   int altura = 1, i = 1;
	   return completoAux(i, altura);
   }
...
private:
   ...
   void flotar(int i) {
      ...
      int hueco = i;
      while (hueco != 1 && antes(elem, array[hueco / 2])) { //linieal en altura del árbol -> log N (N nodos)
